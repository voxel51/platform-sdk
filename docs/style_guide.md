...
This project is implemented in Python, and we require all contributed code to
adhere to our style. Our priority is *consistency*, so that developers can
quickly ingest and understand the entire codebase. When in doubt, follow
the existing style in the module you are contributing to.
...
```python
from voxel51.platform.task import TaskManager, \
                                  TaskFailureType
```
